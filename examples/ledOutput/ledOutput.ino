...
void setup(void)
{
  ...
  参数mode 如下参数是可用的：将引脚设置为输入（INPUT）、输出（OUTPUT）、输入上拉（PULL_PULLUP）模式
  */
  mcp.pinMode(/*p = */mcp.eGPA0, /*mode = */OUTPUT);
}
...
void loop(void)
{
  ...
  eGPB0  eGPB1  eGPB2  eGPB3  eGPB4  eGPB5  eGPB6  eGPB7
   8    9   10   11   12   13   14   15
   参数level，如下参数是可用的：将引脚设置为高电平（HIGH）、低电平（LOW）
  */
  mcp.digitalWrite(/*p = */mcp.eGPA0, /*level = */HIGH);
  delay(1000);
